...
const { Pool } = require('pg'); // For PostgreSQL pool
...
app.post('/api/referral/redeem', async (req, res) => {
  try {
    ...
    console.log("DEBUG: Running query: SELECT * FROM users WHERE email = $1", email);
    ...
    await pool.query('UPDATE users SET points = $1 WHERE user_id = $2', [newPoints, user.user_id]);
    ...
    await pool.query(
      ...
      [user.user_id, `redeem-${redeemType}`, -pointsToRedeem]
    );
...
app.post('/api/referral/cancel-redeem', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE users SET points = $1, last_discount_code = NULL, discount_code_id = NULL WHERE email = $2',
      [newPoints, email]
    );
 ...
 * Creates a $15 welcome discount for referred users (no points deducted)
...
app.post('/api/referral/create-welcome-discount', async (req, res) => {
  try {
    ...
    console.log(`DEBUG: Creating $15 welcome discount for referred user: ${email}`);

    // Create a $15 discount code
    const { code: discountCode, discountId } = await createShopifyDiscountCode('15', 0);

    console.log(`DEBUG: Created welcome discount code: ${discountCode} for ${email}`);

    // Log this action in user_actions if user exists
    const result = await pool.query('SELECT * FROM users WHERE email = $1', [email]);
    if (result.rows.length > 0) {
      const user = result.rows[0];
      await pool.query(
        'INSERT INTO user_actions (user_id, action_type, points_awarded) VALUES ($1, $2, $3)',
        [user.user_id, 'referral_welcome_discount', 0]
      );
    }
